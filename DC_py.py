...
"""С помощью GridSearchCV подбираются оптимальные параметры для модели логистической регрессии, используя кросс-валидацию для оценки качества и метрику recall для оптимизации, а затем сохраняется лучшая обученная модель."""
...
Здесь строится ROC-кривая и вычисляется AUC-ROC для оценки качества модели с использованием настроенного порога вероятности, отображая графически соотношение между True Positive Rate и False Positive Rate и численно оценивая общую производительность модели.
"""
...
"""Извлекается и выводится вероятность бронирования (принадлежности к классу 1) для конкретного экземпляра (в данном случае, первого) из DataFrame, содержащего вероятности предсказаний."""
...
"""Загружаются данные из CSV-файла “Final_features1.csv” в DataFrame pandas, а затем выделяются признаки (X) путем удаления столбца “booking_status” и целевая переменная (y) из столбца “booking_status”."""
...
# 2. Разделение данных на обучающую и тестовую выборки (исходных данных)
...
# 4. Применение SMOTE к обучающей выборке *после* стандартизации
...
# 5. Определение параметров для перебора
param_grid = {
    'criterion': ['gini', 'entropy'],
    'max_depth': [None, 5, 10, 15, 20], # добавил больше вариантов
    'min_samples_split': [2, 5, 10, 15], # добавил больше вариантов
    'min_samples_leaf': [1, 2, 4, 6] # добавил больше вариантов
...
# 6. Создание модели DecisionTreeClassifier
...
# 8. Обучение с использованием GridSearchCV
grid_search.fit(over_X_train, over_y_train)

"""Выводятся наилучшие параметры модели дерева решений, найденные в процессе обучения с использованием GridSearchCV."""

# 9. Вывод лучших параметров
...
# 13. Матрица ошибок (Confusion Matrix)
...
# 14. Визуализация матрицы ошибок (опционально)
...
# 4. Применение SMOTE к обучающей выборке
oversample = SMOTE(random_state=77)
over_X_train, over_y_train = oversample.fit_resample(X_train, y_train)
...
# 6. Создание модели GradientBoostingClassifier
...
grid_search = GridSearchCV(estimator=gb_model,
                           ...
                           scoring='recall',      # Метрика оценки (recall)
...
Отчет о классификации (Classification Report): Более подробный отчет, включающий precision, recall, F1-score и support для каждого класса. Эти метрики показывают, насколько хорошо модель предсказывает каждый класс.
"""
...
# 14. Визуализация матрицы ошибок
plt.figure(figsize=(8, 6))
sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues',
            xticklabels=['Class 0', 'Class 1'],
            yticklabels=['Class 0', 'Class 1'])
...
# 6. Создание модели RandomForestClassifier
